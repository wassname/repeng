...
def read_representations(
    ...
    for layer in tqdm.tqdm(hidden_layers):
        ...
        grad_matrix = grads[layer].clone() if grads[layer] is not None else None
        ...
        if method == "svd_gradient":
            ...
            rank = None
            if "_rank" in method:
                # Extract rank from method name like "kfac_steer_rank64"
                import re
                match = re.search(r'_rank(\d+)', method)
                if match:
                    rank = int(match.group(1))
            
            directions[layer] = kfac_contrastive_steering(
                h_pos, h_neg, grad_pos, grad_neg,
                lambda_reg=lambda_reg,
                mode=combo_mode,
                kfac_mode=kfac_mode,
                rank=rank,
